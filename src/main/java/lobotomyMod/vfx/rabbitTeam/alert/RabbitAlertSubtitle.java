 ...
 * @author hoykj
 */
public class RabbitAlertSubtitle extends AbstractGameEffect {
    ...
    public RabbitAlertSubtitle(){
        ...
        this.word = TEXT[2];
        this.gl.setText(LobotomyFontHelper.Font_30, this.word);
    ...
    public void update(){
        ...
        for(int i = 0; i < this.x.size(); i ++){
            this.x.set(i, this.x.get(i) + Gdx.graphics.getDeltaTime() * this.v);
        }
        if(this.x.get(0) > Settings.WIDTH){
            this.x.remove(0);
        }
        if(this.x.get(this.x.size() - 1) > 0){
            this.x.add(this.x.get(this.x.size() - 1)-this.gl.width * 1.15F);
        }

        for(int i = 0; i < this.x2.size(); i ++){
            this.x2.set(i, this.x2.get(i) - Gdx.graphics.getDeltaTime() * this.v);
        }
        if(this.x2.get(0) + this.gl.width < 0){
            this.x2.remove(0);
        }
        if(this.x2.get(this.x2.size() - 1) + this.gl.width < Settings.WIDTH){
            this.x2.add(this.x2.get(this.x2.size() - 1)  + this.gl.width * 1.15F);
        }
    ...
    public void render(SpriteBatch sb) {
        ...
        for (Float aX : this.x) {
            FontHelper.renderSmartText(sb, LobotomyFontHelper.Font_30, this.word, aX, this.y, Settings.WIDTH, 0.0F, this.color2);
        }
        for (Float aX : this.x2) {
            FontHelper.renderSmartText(sb, LobotomyFontHelper.Font_30, this.word, aX, this.y2, Settings.WIDTH, 0.0F, this.color2);
        }
    }
